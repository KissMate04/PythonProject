...
while running:
    ...
    for event in pygame.event.get():
        ...
        if (event.type == pygame.MOUSEBUTTONDOWN and event.button == 1) or (event.type == pygame.KEYDOWN and event.key == pygame.K_SPACE):
            game.projectiles.append(projectile.Projectile(player.x + player.shipsize / 2, player.y, player.shipsize / 2, player.speed, player.damage, "player"))
        if event.type == ENEMY_SHOOT:
            for en in enemy.enemies:
                game.projectiles.append(projectile.Projectile(en.x + en.shipsize/2, en.y+en.shipsize, en.shipsize / 2, en.speed, en.damage, en))
